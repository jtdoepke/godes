...
const (
	...
	INTERVAL_CUSTOMERS = 12.00 // Generate new customers roughly every x minites
	SERVICE_TIME       = 12.0
...
var (
	...
	arrivalGen *godes.ExpDistr = godes.NewExpDistr(true)

	// random generator for the patience time time - uniform distribution
	patienceGen *godes.UniformDistr = godes.NewUniformDistr(true)

	// random generator for the  service time - expovariate distribution
	serviceGen *godes.ExpDistr = godes.NewExpDistr(true)

	// true when Counter
	counterAvailable *godes.BooleanControl = godes.NewBooleanControl()
)
